...
@section('title') {{'Cosmic Comfort'}} @endsection
@section('content')
    @include('backend.partials.alert')

    <!-- Start::page-header -->
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <h1 class="page-title fw-semibold fs-18 mb-0">Cosmic Comfort <a href="{{ route('cosmicComfort.create') }}"
                ...
                <div class="card-body">
                    <div class="table-responsive mb-4">
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                            @foreach ($cosmicComfort as $comfort)
                                <tr class="product-list">
                                    ...
                                    <td>{{ $comfort->sku }}</td>
                                    <td>{{ $comfort->upcasinfnsku }}</td>
                                    <td>{{ $comfort->whinventory }}</td>
                                    <td>{{ $comfort->amazoninventory }}</td>
                                    <td>{{ $comfort->totalinventory }}</td>
                                    <td>{{ $comfort->ninetydayamazon }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <a href="{{ route('cosmicComfort.edit', $comfort->id) }}" class="btn btn-icon btn-sm btn-info-light"><i
                                                        class="ri-edit-line"></i></a>
                                            <a href="javascript:void(0);" data-bs-toggle="modal"
                                               data-bs-target="#deleteModal{{ $comfort->id }}" data-original-title="Delete" class="btn btn-icon btn-sm btn-danger-light product-btn">
                                ...
                                <!-- Modal Delete -->
                                <div class="modal fade" id="deleteModal{{ $comfort->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <form class="d-inline-block"
                                                      action="{{ route('cosmicComfort.destroy', $comfort->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
