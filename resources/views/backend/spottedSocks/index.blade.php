...
@section('title') {{'Spotted Socks'}} @endsection
@section('content')
    @include('backend.partials.alert')

    <!-- Start::page-header -->
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <h1 class="page-title fw-semibold fs-18 mb-0">Spotted Socks <a href="{{ route('spottedSocks.create') }}"
                ...
                <div class="card-body">
                    <div class="table-responsive mb-4">
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                            @foreach ($spottedSocks as $spsock)
                                <tr class="product-list">
                                    <td class="product-checkbox"><input class="form-check-input" type="checkbox"
                                                                        id="product1" value="" aria-label="..."></td>
                                    <td>{{ $spsock->sku }}</td>
                                    <td>{{ $spsock->fnsku }}</td>
                                    <td>{{ $spsock->whinventory }}</td>
                                    <td>{{ $spsock->amazoninventory }}</td>
                                    <td>{{ $spsock->totalinventory }}</td>
                                    <td>{{ $spsock->thirtydayssales }}</td>
                                    <td>{{ $spsock->ninetydayamazon }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <a href="{{ route('spottedSocks.edit', $spsock->id) }}" class="btn btn-icon btn-sm btn-info-light"><i
                                                        class="ri-edit-line"></i></a>
                                            <a href="javascript:void(0);" data-bs-toggle="modal"
                                               data-bs-target="#deleteModal{{ $spsock->id }}" data-original-title="Delete" class="btn btn-icon btn-sm btn-danger-light product-btn">
                                ...
                                <!-- Modal Delete -->
                                <div class="modal fade" id="deleteModal{{ $spsock->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <form class="d-inline-block"
                                                      action="{{ route('spottedSocks.destroy', $spsock->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
