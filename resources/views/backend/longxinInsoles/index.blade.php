...
@section('title') {{'Longxin Insoles'}} @endsection
@section('content')
    @include('backend.partials.alert')

    <!-- Start::page-header -->
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <h1 class="page-title fw-semibold fs-18 mb-0">Longxin Insoles <a href="{{ route('longxinInsoles.create') }}"
                ...
                <div class="card-body">
                    <div class="table-responsive mb-4">
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                            @foreach ($longxinInsoles as $insole)
                                <tr class="product-list">
                                    ...
                                    <td>{{ $insole->sku }}</td>
                                    <td>{{ $insole->fnsku }}</td>
                                    <td>{{ $insole->whinventory }}</td>
                                    <td>{{ $insole->amazoninventory }}</td>
                                    <td>{{ $insole->totalinventory }}</td>
                                    <td>{{ $insole->ninetydayamazon }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <a href="{{ route('longxinInsoles.edit', $insole->id) }}" class="btn btn-icon btn-sm btn-info-light"><i
                                                        class="ri-edit-line"></i></a>
                                            <a href="javascript:void(0);" data-bs-toggle="modal"
                                               data-bs-target="#deleteModal{{ $insole->id }}" data-original-title="Delete" class="btn btn-icon btn-sm btn-danger-light product-btn">
                                ...
                                <!-- Modal Delete -->
                                <div class="modal fade" id="deleteModal{{ $insole->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <form class="d-inline-block"
                                                      action="{{ route('longxinInsoles.destroy', $insole->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
