...
@section('title') {{'ZeroSock'}} @endsection
@section('content')
    @include('backend.partials.alert')

    <!-- Start::page-header -->
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <h1 class="page-title fw-semibold fs-18 mb-0">ZeroSock <a href="{{ route('zerosock.create') }}"
                ...
                <div class="card-body">
                    <div class="table-responsive mb-4">
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                            @foreach ($zerosock as $sock)
                                <tr class="product-list">
                                    <td class="product-checkbox"><input class="form-check-input" type="checkbox"
                                                                        id="product1" value="" aria-label="..."></td>
                                    <td>{{ $sock->sku }}</td>
                                    <td>{{ $sock->whinventory }}</td>
                                    <td>{{ $sock->amazoninventory }}</td>
                                    <td>{{ $sock->totalinventory }}</td>
                                    <td>{{ $sock->thirtydayssales }}</td>
                                    <td>{{ $sock->ninetydayamazon }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <a href="{{ route('zerosock.edit', $sock->id) }}" class="btn btn-icon btn-sm btn-info-light"><i
                                                        class="ri-edit-line"></i></a>
                                            <a href="javascript:void(0);" data-bs-toggle="modal"
                                               data-bs-target="#deleteModal{{ $sock->id }}" data-original-title="Delete" class="btn btn-icon btn-sm btn-danger-light product-btn">
                                ...
                                <!-- Modal Delete -->
                                <div class="modal fade" id="deleteModal{{ $sock->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <form class="d-inline-block"
                                                      action="{{ route('zerosock.destroy', $sock->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
