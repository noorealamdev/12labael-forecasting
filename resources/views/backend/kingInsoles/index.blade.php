...
@section('title') {{'King Insoles'}} @endsection
@section('content')
    @include('backend.partials.alert')

    <!-- Start::page-header -->
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <h1 class="page-title fw-semibold fs-18 mb-0">King Insoles <a href="{{ route('kingInsoles.create') }}"
                        ...
                        <table class="table text-nowrap table-bordered">
                            ...
                            @foreach ($kingInsoles as $sole)
                                <tr class="product-list">
                                    ...
                                    <td>{{ $sole->sku }}</td>
                                    <td>{{ $sole->upcasinfnsku }}</td>
                                    <td>{{ $sole->whinventory }}</td>
                                    <td>{{ $sole->amazoninventory }}</td>
                                    <td>{{ $sole->totalinventory }}</td>
                                    <td>{{ $sole->ninetydayamazon }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <a href="{{ route('kingInsoles.edit', $sole->id) }}" class="btn btn-icon btn-sm btn-info-light"><i
                                                        class="ri-edit-line"></i></a>
                                            <a href="javascript:void(0);" data-bs-toggle="modal"
                                               data-bs-target="#deleteModal{{ $sole->id }}" data-original-title="Delete" class="btn btn-icon btn-sm btn-danger-light product-btn">
                                ...
                                <!-- Modal Delete -->
                                <div class="modal fade" id="deleteModal{{ $sole->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <form class="d-inline-block"
                                                      action="{{ route('kingInsoles.destroy', $sole->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
